...
class AliyunAsrWakeSession(
    ...
    private val aiChatListener = object : ILargeDataResponse<AiChatResponse> {
    ...
    private val controlCb = object : ILargeDataResponse<GlassModelControlResponse> {
        // 控制类回调，这里不处理具体数据。
        override fun parseData(cmdType: Int, response: GlassModelControlResponse?) = Unit
    }

    private val decodeCb = object : OnDecodeStreamCallback {
        // Opus 解码后输出 PCM 数据。
        ...
        override fun onError(code: Int, message: String?) {
            logStatus("Opus decode error: code=$code message=$message")
        }
    }
    ...
    override fun onNuiAudioStateChanged(state: Constants.AudioState) {
        logStatus("onNuiAudioStateChanged: $state")
    }
    ...
    override fun onNuiNeedAudioData(buffer: ByteArray, len: Int): Int {
        return audioQueue.read(buffer, len)
    }
    ...
    override fun onNuiEventCallback(
        ...
        when (event) {
            Constants.NuiEvent.EVENT_ASR_PARTIAL_RESULT -> {
                // optional: suppress to reduce log noise
            }
            ...
            Constants.NuiEvent.EVENT_ASR_ERROR -> {
                logStatus("ASR error code=$resultCode")
                scheduleStop("asr_error")
            }
            Constants.NuiEvent.EVENT_MIC_ERROR -> {
                logStatus("MIC error (no audio data)")
